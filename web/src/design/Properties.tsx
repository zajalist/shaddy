...
export type PropertiesPanelProps = {
  ...
  /** Live renderer FPS, polled by DesktopApp. */
  fps?: number;
};

export const PropertiesPanel = ({ selectedCard, selectedIndex, fps = 60 }: PropertiesPanelProps) => (
  <div
    style={{
      flex: '1 1 auto',
...
const GlobalProps = ({ fps }: { fps: number }) => {
  const recipe = useCardsStore((s) => s.recipe);
  const setRecipe = useCardsStore((s) => s.setRecipe);
  ...
  return (
    <>
      ...
      <div style={{ padding: '2px 14px 14px', font: `400 12px ${TYPE.body}`, color: SHADE.textDim, lineHeight: 1.55 }}>
        Select a block in the canvas to edit it. Or set the canvas and export defaults below.
      </div>
      ...
      <div style={{ padding: '0 14px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
            <span style={{ font: `600 10.5px ${TYPE.body}`, color: SHADE.textDim, letterSpacing: '0.14em', textTransform: 'uppercase' }}>
              Export resolution
            </span>
            <span style={{ font: `500 11px ${TYPE.bodyMono}`, color: SHADE.textFaint }}>
      ...
      <div style={{ padding: '0 14px 14px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <button
          ...
          style={{
            ...
            background: canvas.transparentExport ? SHADE.surface3 : SHADE.surface1,
            border: `1px solid ${canvas.transparentExport ? SHADE.borderHi : SHADE.border}`,
            borderRadius: 7,
            color: canvas.transparentExport ? SHADE.text : SHADE.textDim,
            cursor: 'pointer',
