...
export const TopBar = ({ tempo = 120, tapping = false }: { tempo?: number; tapping?: boolean }) => (
  ...
  >
    ...
    {/* right cluster: tools + share + sign-in */}
    <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8, position: 'relative', zIndex: 1 }}>
      ...
      >
        <Icon name="share" size={12} color="#1a1208" /> Share
      </button>
      <a
        href="#"
        style={{
...
const ToolbarButton = ({ icon, label }: { icon: string; label: string }) => (
  <button
    style={{
      background: 'transparent', color: SHADE.topbarText,
      border: '1px solid rgba(255,255,255,0.10)', borderRadius: 3,
      padding: '6px 11px', font: `500 12px ${TYPE.body}`,
      display: 'flex', alignItems: 'center', gap: 7, cursor: 'pointer',
      letterSpacing: '0.02em',
    }}
  >
    <Icon name={icon} size={13} color={SHADE.topbarDim} />
    <span>{label}</span>
  </button>
);
...
const PaletteCategoryHeader = ({
  cat, count,
}: { cat: (typeof CATEGORIES)[keyof typeof CATEGORIES]; count: number }) => (
  <div
    style={{
      display: 'flex', alignItems: 'center', gap: 9,
      margin: '2px 6px 8px',
...
export const Palette = ({ width = 240 }: { width?: number }) => {
  const grouped = (Object.keys(CATEGORIES) as Array<keyof typeof CATEGORIES>).map((k) => ({
    k,
    blocks: BLOCK_LIB.filter((b) => b.cat === k),
  }));
  return (
    <div
      style={{
