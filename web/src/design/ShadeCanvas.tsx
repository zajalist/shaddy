...
export type ShaderVariant = 'plasma' | 'ripple' | 'swirl' | 'flow';
...
const FRAGMENT_SRC: Record<ShaderVariant, string> = {
  ...
  `,
  // Calm, classy domain-warped flow in the brand's warm palette — no harsh
  // radial spokes. Soft ember/gold over a deep base with a teal undertone.
  flow: `
    precision highp float;
    uniform vec2 R; uniform float T;
    ...
    void main(){
      ...
      vec2 w = vec2(fbm(uv*1.4 + q*1.8 + vec2(1.7, 9.2) + t*1.3),
                    fbm(uv*1.4 + q*1.8 + vec2(8.3, 2.8) - t));
      float f = fbm(uv*1.4 + w*2.0);
      vec3 base  = vec3(0.035, 0.030, 0.055);
      vec3 teal  = vec3(0.09, 0.34, 0.42);
      vec3 ember = vec3(0.95, 0.45, 0.13);
      vec3 gold  = vec3(0.98, 0.78, 0.30);
      vec3 col = mix(base, teal, smoothstep(0.05, 0.65, f));
      col = mix(col, ember, smoothstep(0.40, 1.0, f + 0.25*w.x));
      col = mix(col, gold, 0.6*smoothstep(0.65, 1.15, f));
      col *= 1.0 - 0.5*dot(uv, uv);   // soft vignette
